...
map.on('load', () => {
    ...
    loadMapData(); // ✅ Load data only AFTER the map is ready
});
...
function updateYear(year) {
    ...
    if (filteredFeatures.length === 0) {
        console.warn("⚠️ No features found for this year.");
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    yearSlider.addEventListener('input', (e) => {
        ...
        yearLabel.textContent = `Year: ${selectedYear}`; // ✅ Update the label
        updateYear(selectedYear); // ✅ Call function to update map
    });
